...
import csv
import torch
from mlep.upstream.util import Logger, printSet, get_device
from mlep.upstream.validate import validate
from mlep.networks.resnet import resnet50
from mlep.options.test_options import TestOptions
import mlep.networks.resnet as resnet
import numpy as np
import random
...
seed_torch(100)
# This file used to sit at the repo root, so 'datasets/' was one level up from it.
# Under src/mlep/upstream/ it is four, hence REPO_ROOT. MLEP_DATASETS_ROOT overrides
# it for anyone keeping the (60 GB) datasets on another volume.
REPO_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(
    os.path.dirname(os.path.abspath(__file__)))))
DATASETS_ROOT = os.environ.get(
    'MLEP_DATASETS_ROOT', os.path.join(REPO_ROOT, 'datasets', 'TestDatasets'))
DetectionTests = {
...
# Mirrors mlep.harness.data.EVAL_SCENARIOS so the two harnesses are comparable.
CORRUPTIONS = {
    'clean': dict(),
