...
def binary_dataset(opt, root):
    ...
    dset = datasets.ImageFolder(
            ...
            transforms.Compose([
                rz_func,
                # Image corruptions (blur / jpeg / webp). This is a no-op when all
                # of blur_prob / jpg_prob / webp_prob are 0, so clean runs and the
                # pretrained test.py path are unaffected; it is what makes the
                # robustness scenarios in mlep/experiments/windows.py actually corrupt.
...
def data_augment(img, opt):
    ...
    if random() < getattr(opt, 'jpg_prob', 0):
        method = sample_discrete(opt.jpg_method)
        qual = sample_discrete(opt.jpg_qual)
        img = jpeg_from_key(img, qual, method)

    # webp is optional: opt objects that predate it (e.g. TestOptions) simply
    # have webp_prob=0 via getattr, so this stays a no-op for them.
    if random() < getattr(opt, 'webp_prob', 0):
        qual = sample_discrete(getattr(opt, 'webp_qual', [80]))
        img = webp_from_img(img, qual)
...
def webp_from_img(img, compress_val):
    out = BytesIO()
    Image.fromarray(img).save(out, format='webp', quality=compress_val)
