...
Run it with
    python -m mlep.experiments.windows --experiment entropy --dataroot ... --out results/entropy_<stamp>.txt
or  ./run.sh entropy
...
def extract_features(args, split, is_train, aug, batches, label):
    ...
    from mlep.harness.data import make_loader, make_opt
...
def run_entropy_experiment(args, device, gpu_name, use_amp):
    ...
    from mlep.harness.evaluate import run_config
