...
Run it with
    python -m mlep.experiments.windows --experiment mlep_degradation --dataroot ... \
...
Blur reuses mlep.data.datasets.gaussian_blur, i.e. scipy.ndimage.gaussian_filter with
...
# released front-end (mlep.experiments.windows.CONFIGS['baseline_2x2'], the hardcoded
...
class DegradationDataset(torch.utils.data.Dataset):
    ...
    Mirrors mlep.data.datasets.binary_dataset's transform chain (resize -> degrade ->
...
def _mid_train_eval(model, name, step, args, device, use_amp, val_set, recal_loader):
    ...
    from mlep.harness.data import make_loader
...
def run_degradation_config(name, cfg, args, device, use_amp, val_sets):
    ...
    Deliberately the same shape as mlep.harness.evaluate.run_config -- same fixed
...
def run_degradation_experiment(args, device, gpu_name, use_amp):
    from mlep.harness.report import write_report
