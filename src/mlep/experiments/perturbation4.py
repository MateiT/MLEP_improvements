"""Full retrain of a 4-output MLEP variant on the half-size ProGAN train set.
    ...
    python python -m mlep.experiments.perturbation4 --dataroot datasets/TrainDatasets \
...
the report header:
  ...
  1. BatchNorm statistics are re-estimated before every validation and before
     every save (see mlep.harness.model.recalibrate_bn). Without it the stored
...
class Perturbation4Dataset(torch.utils.data.Dataset):
    ...
    Transform order mirrors mlep.data.datasets.binary_dataset and
    mlep.experiments.degradation.DegradationDataset: resize -> degrade -> crop ->
...
def main():
    ...
    # bare state_dict -- python -m mlep.evaluation.trained_model and the MLEP_SPAI wrapper
