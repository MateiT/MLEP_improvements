...
\begin{quote}\small
\textbf{Geometria de evaluare.} Fişierul \texttt{test.py} publicat setează
...
suplimentar \texttt{unique}, care înlocuieşte entropia cu o simplă numărare a
valorilor distincte din fereastră, adică cardinalul mulţimii de valori în locul
unei funcţii ponderate a distribuţiei.
...
De remarcat, de asemenea, că \texttt{unique} performează identic cu entropia
Shannon ($0.9980$ faţă de $0.9979$). Întrucât \texttt{unique} elimină complet
multiplicităţile şi păstrează doar numărul de valori distincte, aceasta este
prima indicaţie că \emph{funcţionala precisă} nu poartă practic nicio informaţie
dincolo de partiţia grosieră --- o constatare pe care
Secţiunea~\ref{sec:exp-entropy} o confirmă mult mai temeinic.
...
\begin{description}
  \item[Nepotrivirea intervalului dinamic.] Entropia Shannon este mărginită de
    $\log_2 w^2$: $2.0$ biţi pentru $2\times2$, dar $\approx5.17$ pentru
    $6\times6$. Canalele cu ferestre late intră deci în \texttt{conv1} cu un
    interval dinamic de $2.5\times$ mai mare decât cele înguste. \texttt{conv1}
    are \texttt{bias=False}, iar \texttt{bn1} normalizează cele 64 de canale de
    \emph{ieşire} \emph{după} ce convoluţia a însumat deja intrările, aşa că
    nimic în aval nu anulează acest dezechilibru. \texttt{normalize\_entropy}
    împarte fiecare hartă la propriul $\log_2 K$.
  \item[Dezalinierea spaţială.] O hartă cu fereastră $w$ are lăţimea $H-w+1$,
    deci cele trei hărţi diferă ca dimensiune, iar calea implicită le
    reeşantionează biliniar pe grila $2\times2$. Acest lucru este de două ori
    păgubitor: aliniază extremităţile exterioare ale hărţilor, nu pixelii pe care
    îi descriu, ceea ce le \emph{întinde} în loc să le deplaseze (eroare măsurată
    $\approx0$ în centrul imaginii, crescând la $\approx1.6$\,px pentru fereastra
    $6\times6$ lângă margine), iar reeşantionarea estompează exact structura de
    frecvenţă înaltă care constituie întregul semnal. O reţea convoluţională
    poate absorbi o deplasare constantă în poziţiile filtrelor învăţate, dar nu
    şi o deformare dependentă de poziţie, întrucât ponderile sunt partajate.
    Varianta \texttt{pad} completează prin replicare cu $(w-w_{\min})/2$ în
    schimb, ceea ce este exact, fără interpolare, şi s-a măsurat că a costat
...
Ambele se reduc la Shannon când $\alpha, q \to 1$; $\alpha > 1$ accentuează
valoarea dominantă, $\alpha < 1$ pe cele rare. Am testat de asemenea entropia de
\emph{permutare} (tipare ordinale Bandt--Pompe), care este categoric diferită: ea
elimină magnitudinile şi păstrează doar \emph{ordinea de rang} a pixelilor,
fiind astfel invariantă la orice transformare monotonă a intensităţii. Întrucât o
singură fereastră citită ca o secvenţă produce exact un tipar ordinal (şi deci
entropie zero), definim populaţia de tipare din interiorul unei ferestre ca
totalitatea subsecvenţelor de ordin 3 ale liniilor şi coloanelor sale; acest
lucru necesită $w \geq 3$.
...
şi decurge din Tabelul~\ref{tab:2x2}. La $w=2$ fereastra admite doar cinci
partiţii posibile ale celor patru valori. \emph{Orice} funcţională de entropie
este o funcţie doar de partiţie, deci fiecare funcţională din
...
\begin{table}[t]
  ...
  \begin{tabular}{llrrl}
    ...
    1 & \texttt{unique} în loc de Shannon        & $+0.0001$ & $-0.0062$ & niciun efect \\
    \midrule
